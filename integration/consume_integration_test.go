...
func (ti *testConsumeIntegration) test(t *testing.T) {
	...
	go func() {
		...
		produceBatch(t, ctx, producer, "", mpulsar.NewJSONSerializer(), instanceLabels)
		produceBatch(t, ctx, producer, "", mpulsar.NewJSONSerializer(), instanceLabels)
		produceBatch(t, ctx, producer, "", mpulsar.NewJSONSerializer(), instanceLabels)
		produceBatch(t, ctx, producer, "", mpulsar.NewJSONSerializer(), instanceLabels)
		producer.Close()
	...
	for {
		...
		if instancesToBeReceivedByTimestamp.finishedSamples() == samplesEach {
			t.Logf("%s: received %d samples per series", time.Now(), samplesEach)
			break
		}
	}

	t.Logf("%s: adapter cancel", time.Now())
	adapterCancel()
	t.Logf("%s: wg wait", time.Now())
	wg.Wait()
}
